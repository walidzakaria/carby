...
def eta_list_invoices(request):
    if request.method == 'GET':
        ...
        date_filter = 'qu.date_issued BETWEEN %s AND %s'
        query = f'''
                SELECT DISTINCT
                    ...
                    qu.date_issued AS "DateIssued", qu.id AS "InternalId", '' AS "PurchaseOrderReference",
    ...
    if request.method == 'GET':
        query = '''
                SELECT
                    ...
                    qu.date_issued AS "DateIssued", qu.id AS "InternalId", '' AS "PurchaseOrderReference", COALESCE(qu.name, '') AS "PurchaseOrderDescription",
